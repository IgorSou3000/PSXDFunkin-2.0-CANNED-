...
static void Stage_TimerTick(void)
{
	...
	if (stage.song_step >= 0) //If song starts decrease the timer
	{
			...
			if (stage.timer != 0)
   			stage.timer = Audio_GetLength(stage.stage_def->music_track) - (stage.song_time >> FIXED_SHIFT) - 1; //Seconds (ticks down)
  }
...
static void Stage_DrawHealthBar(u32 color, s8 ox)
{
  ...
  healthcolor.b = ((color) & 0xFF) / 2;        // Extract the BB byte
	...
	RECT_FIXED health_dst = {FIXED_DEC(-100,1), (SCREEN_HEIGHT2 - 30) << FIXED_SHIFT, healthw << FIXED_SHIFT, FIXED_DEC(6,1)};
	...
	if (stage.stage_def->week & 0x80)
  {
  	//Use mod path convention
		static const char *mod_format[] = {
			"\\BONUS1\\B1.%d%c.CHT;1", //Bonus1 (Sonic.exe)
			"\\BONUS2\\B2.%d%c.CHT;1", //Bonus2 (Ourple)
			"\\BONUS3\\B3.%d%c.CHT;1", //Bonus3 (God Feast)
		};
		
		sprintf(chart_path, mod_format[stage.stage_def->week % 128], stage.stage_def->week_song, "ENH"[stage.stage_diff]);
  }
...
static void Stage_LoadState(void)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		strcpy(stage.player_state[i].accuracy_text, "| Rating: ?");
...
{
	//Set camera focus
	if (stage.cur_section->flag & SECTION_FLAG_OPPFOCUS || stage.stage_id == StageId_2_4)
	...
	switch (stage.state)
	{
		...
		{		
  		...
  		Events_StartEvents();
			...
			//Scroll camera
			if ((stage.cur_section->flag & SECTION_FLAG_OPPFOCUS && stage.opponent != NULL) || (stage.player == NULL)  || (stage.stage_id == StageId_2_4 && stage.song_step < 33))
			...
			for (int i = 0; i < ((stage.mode >= StageMode_2P) ? 2 : 1); i++)
			{
				...
				{
					if (this->accuracy != 0)
						sprintf(this->accuracy_text, "| Rating: (%d%%)", this->accuracy);
					else
						strcpy(this->accuracy_text, "| Rating: ?");
